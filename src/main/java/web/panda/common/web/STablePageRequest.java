...
public class STablePageRequest {
    ...
    public Pageable getPageable()
    {
        ...
        if(StringUtils.isNotBlank(sortField))
        {
            ...
            Sort pageSort = Sort.by(Direction.DESC,sortField);
            //否则 new 升序  排序对象Sort
            if(StringUtils.isNotBlank(sortKind)){
                if(sortKind.contains("asc")){
                    pageSort = Sort.by( Direction.ASC,sortField);
                }
            }
